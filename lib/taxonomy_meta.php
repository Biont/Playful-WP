...
/*
  ...
  License: GPL2+
 */
if ( !class_exists( 'PlayfulTaxonomyMeta' ) ) {

    class PlayfulTaxonomyMeta {
        ...
        function load_edit_page() {
            ...
            if (
                    'edit-tags' != $screen->base || empty( $_GET[ 'action' ] ) || 'edit' != $_GET[ 'action' ] || !in_array( $screen->taxonomy, $this->_taxonomies )
            ) {
                return;
            }
        ...
        function add() {
            ...
            foreach ( get_taxonomies() as $tax_name ) {
                if ( in_array( $tax_name, $this->_taxonomies ) ) {
                    add_action( $tax_name . '_edit_form', array( $this, 'show_table' ), 9, 2 );
                    add_action( $tax_name . '_add_form_fields', array( $this, 'show_divs' ), 9, 2 );
                }
            }
        }

        // Show meta fields
        function show_table( $tag, $taxonomy ) {
            ...
            foreach ( $this->_fields as $field ) {
                ...
                call_user_func( array( $this, 'show_field_' . $field[ 'type' ] ), $field, $meta, 'table' );
        ...
        function show_divs( $tag ) {
            ...
            foreach ( $this->_fields as $field ) {
                ...
                call_user_func( array( $this, 'show_field_' . $field[ 'type' ] ), $field, $meta, 'divs' );
...
//            echo '</table>';
        ...
        function show_field_begin( $field, $meta, $type ) {
            if ( $type == 'table' ) {
        ...
        function show_field_end( $field, $meta, $type ) {
            if ( $type == 'table' ) {

                echo $field[ 'desc' ] ? "<br><span class='description'>{$field[ 'desc' ]}</span></td>" : '</td>';
            } elseif ( $type === 'divs' ) {
                echo $field[ 'desc' ] ? "<br><span class='description'>{$field[ 'desc' ]}</span>" : '';
            }
        }

        function show_field_text( $field, $meta, $type ) {
            $this->show_field_begin( $field, $meta, $type );
            echo "<input type='text' name='{$field[ 'id' ]}' id='{$field[ 'id' ]}' value='$meta' style='{$field[ 'style' ]}'>";
            $this->show_field_end( $field, $meta, $type );
        }

        function show_field_textarea( $field, $meta, $type ) {
            $this->show_field_begin( $field, $meta, $type );
            echo "<textarea name='{$field[ 'id' ]}' cols='60' rows='15' style='{$field[ 'style' ]}'>$meta</textarea>";
            $this->show_field_end( $field, $meta, $type );
        }

        function show_field_select( $field, $meta, $type ) {
            if ( !is_array( $meta ) )
                $meta = ( array ) $meta;
            $this->show_field_begin( $field, $meta, $type );
        ...
        function show_field_radio( $field, $meta, $type ) {
            $this->show_field_begin( $field, $meta, $type );
            $html = array();
            foreach ( $field[ 'options' ] as $key => $value ) {
        ...
        function show_field_file( $field, $meta, $type ) {
            if ( !is_array( $meta ) )
                $meta = ( array ) $meta;
        ...
        function show_field_image( $field, $meta, $type ) {
            if ( !is_array( $meta ) )
                $meta = ( array ) $meta;
        ...
        function show_field_color( $field, $meta, $type ) {
            if ( empty( $meta ) )
                $meta = '#';
            $this->show_field_begin( $field, $meta, $type );
        ...
        function show_field_checkbox_list( $field, $meta, $type ) {
            if ( !is_array( $meta ) )
                $meta = ( array ) $meta;
            $this->show_field_begin( $field, $meta, $type );
            $html = array();
        ...
        function show_field_date( $field, $meta, $type ) {
            $this->show_field_text( $field, $meta, $type );
        }

        function show_field_time( $field, $meta, $type ) {
            $this->show_field_text( $field, $meta, $type );
        }
